...
<form action="process/projet.php" method="POST" onSubmit="return JSformProjet();">
...
<table border="0" cellpadding="0" cellspacing="2" width="100%">
   ...
   <tr>
		<td>{#winProjet_nomProjet#}</td>
		<td><input name="nom" id="nom" type="text" size="30" maxlength="30" value="{$projet.nom}" /></td>
   </tr> 
<tr>
	<td>{#winProjet_groupe#} :</td>
	<td>
   ...
   <tr>
			...
			{if $smarty.const.CONFIG_PROJECT_COLORS_POSSIBLE neq ""}
				<select name="couleur">
					{foreach from=","|explode:$smarty.const.CONFIG_PROJECT_COLORS_POSSIBLE item=couleurTmp}
						<option value="{$couleurTmp}" style="background-color:{$couleurTmp}">{$couleurTmp}</option>
					{/foreach}
				</select>
	...
	<tr>
		...
		<td>
			<input type="submit" value="{#winProjet_valider#|escape:"html"}" class="btn" />
		</td>
	</tr>
</table>
</form>