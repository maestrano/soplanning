...
function ajoutProjet($origine) {
	...
	$objResponse->addScript('winFormPeriode.destroy();');
	$objResponse->addScript('winFormPeriode = new Window({className: "charte1", title: "' . addslashes($smarty->get_config_vars('ajax_titreCreationProjet')) . '", width:430, height:330, destroyOnClose: true, recenterAuto:true});');
	$objResponse->addScript('winFormPeriode.setHTMLContent("' . xajaxFormat($smarty->getHtml('projet_form.tpl')) . '");');
	$objResponse->addScript('winFormPeriode.showCenter();');
...
function modifProjet($projet_id, $origine) {
	...
	if($user->checkDroit('planning_modify_own_project') && $projet->createur_id != $user->user_id) {
		$objResponse->addAlert(addslashes($smarty->get_config_vars('ajax_droitsInsuffisants')));
		$objResponse->addScript('location.reload();');
		return $objResponse->getXML();
	}
	...
	$objResponse->addScript('winFormPeriode.destroy();');
	$objResponse->addScript('winFormPeriode = new Window({className: "charte1", title: "' . addslashes($smarty->get_config_vars('ajax_titreModifProjet')) . '", width:430, height:330, destroyOnClose: true, recenterAuto:true});');
	$objResponse->addScript('winFormPeriode.setHTMLContent("' . xajaxFormat($smarty->getHtml('projet_form.tpl')) . '");');
	$objResponse->addScript('winFormPeriode.showCenter();');
	$objResponse->addScript("myPicker = new jscolor.color(document.getElementById('couleur'), {});myPicker.fromString('" . $projet->couleur . "')");
	$objResponse->addScript('jQuery("#livraison").datepicker();');
...
function ajoutPeriode($dateDebut = '', $user_id = '', $periode_id = '') {
	$objResponse = new xajaxResponse('ISO-8859-1');
	$smarty = new MySmarty();

	$user = new User();
	if($user->chargerUserFromSession() !== TRUE || $user->checkDroit('planning_readonly')) {
		$objResponse->addAlert(addslashes($smarty->get_config_vars('ajax_droitsInsuffisants')));
		$objResponse->addScript('location.reload();');
		return $objResponse->getXML();
	}
	...
	if($user->checkDroit('planning_modify_all') || $user->checkDroit('planning_modify_own_project')) {
		$listeUsers->db_load(array('visible_planning', '=', 'oui'), array('nom' => 'ASC'));
	} elseif ($user->checkDroit('planning_modify_own_task')) {
		$listeUsers->db_load(array('user_id', '=', $user->user_id), array('nom' => 'ASC'));
	}
	...
	$objResponse->addScript('winFormPeriode.destroy();');
	$objResponse->addScript('winFormPeriode = new Window({className: "charte1", title: "' . addslashes($smarty->get_config_vars('ajax_titreGestionPeriode')) . '", width:520, height:430, destroyOnClose: true, recenterAuto:true, onClose:Reloader.closeWindow});');
	$objResponse->addScript('winFormPeriode.setHTMLContent("' . xajaxFormat($smarty->getHtml('periode_form.tpl')) . '");');
	$objResponse->addScript('winFormPeriode.showCenter();');
	$objResponse->addScript('attachAutoResizeEvents("notes");');
...
function modifPeriode($periode_id) {
	...
	$user = new User();
	if($user->chargerUserFromSession() !== TRUE || $user->checkDroit('planning_readonly') || ($user->checkDroit('planning_modify_own_project') && $projet->createur_id != $user->user_id)) {
		$objResponse->addAlert(addslashes($smarty->get_config_vars('ajax_droitsInsuffisants')));
		$objResponse->addScript('location.reload();');
		return $objResponse->getXML();
	}
...
function checkProjetId($newProjet_id, $currentProjet_id) {
	...
	if($projetTest->db_loadSQL($sql)) {
		...
		$objResponse->addAssign('divStatutCheckProjetId', 'innerHTML', '<img src="images/ok.gif" width="12" height="12" border="0">');
	}
...
function moveCasePeriode($casePeriode, $jourCible, $copie = false) {
	...
	if($userCible->db_load(array('user_id', '=', $chaines2[1]))) {
		...
		if($user->checkDroit('planning_modify_own_task') && $userCible->user_id != $user->user_id) {
		...
		if(!$projetCible->db_load(array('projet_id', '=', $chaines2[1])) || $user->checkDroit('planning_readonly') || ($user->checkDroit('planning_modify_own_project') && $projetCible->createur_id != $user->user_id)) {
			$objResponse->addAlert(addslashes($smarty->get_config_vars('ajax_deplacementImpossible')));
			$objResponse->addScript('location.reload();');
			return $objResponse->getXML();
		}
		if($user->checkDroit('planning_modify_own_task')) {
...
function filtreProjet($chaine) {
	...
	if ($user->checkDroit('projects_manage_all')) {
			// tous les droits
	} elseif($user->checkDroit('planning_modify_own_project')){
...
function choixPDF() {
	...
	$objResponse->addScript('winFormPeriode.destroy();');
	$objResponse->addScript('winFormPeriode = new Window({className: "charte1", title: "PDF", width:300, height:140, destroyOnClose: true, recenterAuto:true});');
	$objResponse->addScript('winFormPeriode.setHTMLContent("' . xajaxFormat($smarty->getHtml('choix_pdf.tpl')) . '");');
	$objResponse->addScript('winFormPeriode.showCenter();');
	return $objResponse->getXML();
}
...
function choixIcal() {
	...
	$objResponse->addScript('masquerSousMenu("divOptions");');
	$objResponse->addScript('winFormPeriode.destroy();');
	$objResponse->addScript('winFormPeriode = new Window({className: "charte1", title: "ICAL", width:500, height:120, destroyOnClose: true, recenterAuto:true});');
	$objResponse->addScript('winFormPeriode.setHTMLContent("' . xajaxFormat($smarty->getHtml('choix_ical.tpl')) . '");');
	$objResponse->addScript('winFormPeriode.showCenter();');
	return $objResponse->getXML();
}
...
function modifUser($user_id) {
	...
	$objResponse->addScript('winFormPeriode.destroy();');
	$objResponse->addScript('winFormPeriode = new Window({className: "charte1", title: "' . addslashes($smarty->get_config_vars('ajax_ajoutModifuser')) . '", width:820, height:410, destroyOnClose: true, recenterAuto:true});');
	$objResponse->addScript('winFormPeriode.setHTMLContent("' . xajaxFormat($smarty->getHtml('user_form.tpl')) . '");');
	$objResponse->addScript('winFormPeriode.showCenter();');
...
function submitFormUser($user_id, $user_id_origine, $user_groupe_id, $nom, $email, $login, $password, $visible_planningOui, $couleur, $notificationsOui, $envoiMailPwd, $droits) {
	...
	if($user_id_origine == '') {
		...
		if(!is_null($user_form->login) && $userTest->db_load(array('login', '=', $user_form->login))) {
		...
		$userTest = new USer();
		if(!is_null($user_form->login) && $userTest->db_load(array('login', '=', $user_form->login, 'user_id', '<>', $user_form->user_id))) {
			$objResponse->addAlert($smarty->get_config_vars('login_existant'));
			return $objResponse;
		}
	}
...
function modifProfil() {
	...
	$objResponse->addScript('winFormPeriode.destroy();');
	$objResponse->addScript('winFormPeriode = new Window({className: "charte1", title: "' . addslashes($smarty->get_config_vars('ajax_editionProfil')) . '", width:500, height:200, destroyOnClose: true, recenterAuto:true});');
	$objResponse->addScript('winFormPeriode.setHTMLContent("' . xajaxFormat($smarty->getHtml('profil_form.tpl')) . '");');
	$objResponse->addScript('winFormPeriode.showCenter();');
...
function submitFormPeriode($periode_id, $projet_id, $user_id, $date_debut, $conserver_duree, $date_fin, $duree, $nb_jours, $repetition, $dateFinRepetition, $appliquerATous, $statut_tache, $livrable, $titre, $notes, $lien, $user_id2, $user_id3) {
	$objResponse = new xajaxResponse();
	$smarty = new MySmarty();
	...
	$periode->duree = ($duree != '' ? $duree : null);
	...
	if($conserver_duree === 'true') {
		...
		// pas de fin renseignée, on met la journée entière pour la tache
		if($periode->duree == '00:00:00') {
			$periode->duree = '09:00:00';
		}
	}
...
function modifFerie($date_ferie) {
	...
	$objResponse->addScript('winFormPeriode.destroy();');
	$objResponse->addScript('winFormPeriode = new Window({className: "charte1", title: "' . addslashes($smarty->get_config_vars('menuFeries')) . '", width:350, height:150, destroyOnClose: true, recenterAuto:true});');
	$objResponse->addScript('winFormPeriode.setHTMLContent("' . xajaxFormat($smarty->getHtml('ferie_form.tpl')) . '");');
	$objResponse->addScript('winFormPeriode.showCenter();');
...
function modifUserGroupe($user_groupe_id) {
	...
	$objResponse->addScript('winFormPeriode.destroy();');
	$objResponse->addScript('winFormPeriode = new Window({className: "charte1", title: "' . addslashes($smarty->get_config_vars('menuGroupesUsers')) . '", width:400, height:80, destroyOnClose: true, recenterAuto:true});');
	$objResponse->addScript('winFormPeriode.setHTMLContent("' . xajaxFormat($smarty->getHtml('user_group_form.tpl')) . '");');
	$objResponse->addScript('winFormPeriode.showCenter();');
...
function autocompleteTitreTache($projet_id) {
	...
	if($projet_id != '') {
		...
		$jsTitreAutocomplete .= 'jQuery("#titre").autocomplete({source: listeTitres, minLength: 0}).focus(function(){jQuery(this).data("autocomplete").search(jQuery(this).val());});';
