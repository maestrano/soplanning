...
if(isset($_GET['inverserUsersProjets'])) {
	...
	// si changement d'affichage on reinitialise le tri
	$_SESSION['triPlanning'] = 'nom-asc';
} elseif (!isset($_SESSION['inverserUsersProjets'])) {
	$_SESSION['inverserUsersProjets'] = false;
}
...
if ($user->checkDroit('planning_view_team_projects') && !is_null($user->user_groupe_id)) {
...
if($user->checkDroit('planning_view_own_projects')) {
...
while($ligneTmp = $lines->fetch()) {
	...
	$html .= '<tr>' . CRLF;
	$html .= '<th id="tdUser_' . ($nbLine-1) . '" nowrap="nowrap"' . ((!is_null($ligneTmp->couleur) && $ligneTmp->couleur != 'FFFFFF') ? ' style="background-color:#'.$ligneTmp->couleur. ';color:' . buttonFontColor('#' . $ligneTmp->couleur) . '"' : '') . '>&nbsp;' . xss_protect($ligneTmp->nom) . '&nbsp;</th>' . CRLF;
	$tmpDate = clone $dateDebut;
	...
	while ($tmpDate <= $dateFin) {
		...
		if($user->checkDroit('planning_modify_all') || $user->checkDroit('planning_modify_own_project') || $user->checkDroit('planning_modify_own_task')) {
		...
		if (isset($joursOccupes[$tmpDate->format('Y-m-d')])) {
			...
			foreach ($joursOccupes[$tmpDate->format('Y-m-d')] as $jour) {
				...
				if(!$user->checkDroit('planning_modify_all') && !($user->checkDroit('planning_modify_own_project') && $jour['createur_id'] == $user->user_id) && !($user->checkDroit('planning_modify_own_task') && $jour['user_id'] == $user->user_id)) {
					//si pas les droits on affiche curseur interdit
					$html .= 'cursor:url(\'images/interdit.png\'),default';
				}
				if( $_SESSION['inverserUsersProjets']) {
				...
				$html .= '">';
				if($jour['livrable'] == 'oui') {
					$html .= '<img src="images/milestone.png" border="0" style="vertical-align:top" />';
				} else {
					$html .= $nom;
				}
				$html .= '</div>';
