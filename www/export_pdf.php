...
while($lineTmp = $lines->fetch()) {
	...
	$html .= '<tr>' . CRLF;
	$html .= '<th id="tdUser_' . ($nbLine-1) . '" nowrap="nowrap">&nbsp;' . $lineTmp->nom . '&nbsp;</th>' . CRLF;
	$tmpDate = clone $dateDebut;
...
$html = '<page orientation="' . $orientation . '"><style>' . file_get_contents('css/export_pdf.css') . '</style>' . $html . '</page>';
