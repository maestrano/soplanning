...
INSERT INTO `planning_config` VALUES('CURRENT_VERSION', '1.28', 'Internal key for auto upgrade control');
...
CREATE TABLE `planning_projet` (
  ...
  CONSTRAINT `projet_ibfk_1` FOREIGN KEY (`groupe_id`) REFERENCES `planning_groupe` (`groupe_id`) ON DELETE SET NULL ON UPDATE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=latin1 COLLATE=latin1_general_ci;
...
CREATE TABLE `planning_periode` (
  ...
  CONSTRAINT `periode_ibfk_1` FOREIGN KEY (`projet_id`) REFERENCES `planning_projet` (`projet_id`) ON DELETE CASCADE ON UPDATE CASCADE,
  CONSTRAINT `periode_ibfk_2` FOREIGN KEY (`user_id`) REFERENCES `planning_user` (`user_id`) ON DELETE CASCADE ON UPDATE CASCADE
...
# 3. Flush all tables
TRUNCATE `planning_ferie`;
TRUNCATE `planning_groupe`;
TRUNCATE `planning_projet`;
TRUNCATE `planning_user`;
TRUNCATE `planning_periode`;
...
INSERT INTO `planning_user` VALUES ('ADM', null, 'admin', 'admin', 'df5b909019c9b1659e86e0d6bf8da81d6fa3499e', null, 'oui', 'black', '["users_manage_all", "projects_manage_all", "projectgroups_manage_all", "planning_modify_all", "planning_view_all_projects", "parameters_all"]', MD5(RAND()), 'non');
